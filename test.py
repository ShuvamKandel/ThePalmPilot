...
THRESHOLD = 5  # Change this value to adjust sensitivity
...
def move_to_target(x, y):
    """Move the cursor directly to the target coordinates."""
...
def cursor_control():
    ...
    while True:
        ...
        mapped_x = map_value(x, 20, 200, 0, 2560)
        mapped_y = map_value(y, 20, 200, 0, 1260)
        
        print(f"Moving cursor to Mapped X: {mapped_x}, Mapped Y: {mapped_y}")
        move_to_target(mapped_x, mapped_y)
